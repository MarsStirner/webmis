define(["collections/appeals"], function ()
{
	...
	var AppealsHistoryItem = View.extend({
		...
		template:
			'<a {{if requestType == "policlinic" || requestType == "diagnostic"}} href="http://'+POLICLINIC_HOST+'/event/event.html?event_id=${id}&token='+Core.Cookies.get("authToken")+'&role='+Core.Cookies.get("currentRole")+'" target="_blank" {{else}} href="/appeals/${id}/" {{/if}}>№ ${number}</a><span class="Label" style="float:right">${finance.name}</span>' +
