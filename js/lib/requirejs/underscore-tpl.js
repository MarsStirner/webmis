...
define(['underscore', 'text'], function(_, text) {
  ...
  var tpl = {
    ...
    load: function(name, req, onLoadNative, config) {
      var onLoad = function(rawTemplate) {
        var tf, templateFunction;

        tf = _.template(rawTemplate);

        if (config.isBuild) {
          templateFunction = tf.source;
          buildMap[name] = templateFunction;
        } else {
          templateFunction = new Function('obj', 'return ' + tf.source)();
        }

        onLoadNative(templateFunction);
      };
