<?php

namespace Webmis\Models\map;

use \RelationMap;
use \TableMap;


/**
 * This class defines the structure of the 'PersonOrder' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.Webmis.Models.map
 */
class PersonorderTableMap extends TableMap
{

    /**
     * The (dot-path) name of this class
     */
    const CLASS_NAME = 'Webmis.Models.map.PersonorderTableMap';

    /**
     * Initialize the table attributes, columns and validators
     * Relations are not initialized by this method since they are lazy loaded
     *
     * @return void
     * @throws PropelException
     */
    public function initialize()
    {
        // attributes
        $this->setName('PersonOrder');
        $this->setPhpName('Personorder');
        $this->setClassname('Webmis\\Models\\Personorder');
        $this->setPackage('Webmis.Models');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('createDatetime', 'Createdatetime', 'TIMESTAMP', true, null, null);
        $this->addColumn('createPerson_id', 'CreatepersonId', 'INTEGER', false, null, null);
        $this->addColumn('modifyDatetime', 'Modifydatetime', 'TIMESTAMP', true, null, null);
        $this->addColumn('modifyPerson_id', 'ModifypersonId', 'INTEGER', false, null, null);
        $this->addColumn('deleted', 'Deleted', 'BOOLEAN', true, 1, null);
        $this->addColumn('person_id', 'PersonId', 'INTEGER', true, null, null);
        $this->addColumn('date', 'Date', 'DATE', true, null, null);
        $this->addColumn('type', 'Type', 'VARCHAR', true, 64, null);
        $this->addColumn('documentDate', 'Documentdate', 'DATE', true, null, null);
        $this->addColumn('documentNumber', 'Documentnumber', 'VARCHAR', true, 16, null);
        $this->addColumn('documentType_id', 'DocumenttypeId', 'INTEGER', false, null, null);
        $this->addColumn('salary', 'Salary', 'VARCHAR', true, 64, null);
        $this->addColumn('validFromDate', 'Validfromdate', 'DATE', false, null, null);
        $this->addColumn('validToDate', 'Validtodate', 'DATE', false, null, null);
        $this->addColumn('orgStructure_id', 'OrgstructureId', 'INTEGER', false, null, null);
        $this->addColumn('post_id', 'PostId', 'INTEGER', false, null, null);
        // validators
    } // initialize()

    /**
     * Build the RelationMap objects for this table relationships
     */
    public function buildRelations()
    {
    } // buildRelations()

} // PersonorderTableMap
